...
AFXDP<StaticConfig>::AFXDP(const ::mica::util::Config& config)
  ...
  while (1) {
    std::array<PacketBuffer*, 2> bufs;
    uint16_t rcvd = receive(1, bufs.data(), 2);
    if (rcvd == 0)
      continue;
...
void AFXDP<StaticConfig>::init_umems() {
  ...
  for (uint16_t eid = 0; eid < endpoint_count_; eid++) {
    void *bufs = mmap(NULL, NUM_FRAMES * XSK_UMEM__DEFAULT_FRAME_SIZE,
...
void AFXDP<StaticConfig>::release(PacketBuffer* buf) {
  auto xsk = buf->xsk;
  *xsk_ring_prod__fill_addr(&xsk->umem->fq, xsk->idx_fq++) = buf->orig;
...
uint16_t AFXDP<StaticConfig>::receive(EndpointId eid, PacketBuffer** bufs,
  ...
  uint32_t rcvd;
  auto xsk = ei.xsk;
  rcvd = xsk_ring_cons__peek(&xsk->rx, 1, &xsk->idx_rx);
  if (!rcvd) {
    return 0;
  }

  int ret = xsk_ring_prod__reserve(&xsk->umem->fq, rcvd, &xsk->idx_fq);
  while (ret != rcvd) {
    ret = xsk_ring_prod__reserve(&xsk->umem->fq, rcvd, &xsk->idx_fq);
  }

  for (int i = 0; i < rcvd; i++) {
    uint64_t hdrm_addr = \
        xsk_ring_cons__rx_desc(&xsk->rx, xsk->idx_rx)->addr - 64;
    bufs[i] = reinterpret_cast<PacketBuffer*>(
        xsk_umem__get_data(xsk->umem->buffer, hdrm_addr));
    bufs[i]->addr = xsk_ring_cons__rx_desc(&xsk->rx, xsk->idx_rx)->addr;
    bufs[i]->length = xsk_ring_cons__rx_desc(&xsk->rx, xsk->idx_rx++)->len;
    bufs[i]->orig = xsk_umem__extract_addr(bufs[i]->addr);
    bufs[i]->xsk = xsk;
  }
